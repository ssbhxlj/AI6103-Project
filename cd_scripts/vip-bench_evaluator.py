...
method = "vcd"
model_results_file = os.path.join(result_path, f"{method}-{model}-{vipbench_split}.json")
...
grade_file = f'{method}-{model}-{vipbench_split}_{gpt_model}-grade-{num_run}runs.json'
grade_file = os.path.join(result_path, grade_file)

# score results regarding capabilities/capability integration to save
cap_score_file = f'{method}-{model}-{vipbench_split}_{sub_set_name}{gpt_model}-cap-score-{num_run}runs.csv'
cap_score_file = os.path.join(result_path, cap_score_file)
cap_int_score_file = f'{method}-{model}-{vipbench_split}_{sub_set_name}{gpt_model}-cap-int-score-{num_run}runs.csv'
